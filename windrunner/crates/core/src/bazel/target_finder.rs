...
impl BazelTargetFinder {
    ...
    pub fn find_targets_for_file(
        ...
        tracing::debug!(
            "find_targets_for_file: file_path={:?}, workspace_root={:?}",
            file_path,
            workspace_root
        );
        ...
        let relative_path = file_path.strip_prefix(build_dir).map_err(|e| {
            tracing::debug!(
                "Failed to strip prefix: file_path={:?}, build_dir={:?}, error={:?}",
                file_path,
                build_dir,
                e
            );
            ...
            for target in &all_targets {
                ...
                if let Some(crate_ref) = &target.attributes.crate_ref {
                    ...
                    if crate_name == lib_name
                        && !matching_targets.iter().any(|t| t.name == target.name)
                    {
                        matching_targets.push(target.clone());
                    }
                }
    ...
    pub fn find_runnable_target(
        ...
        if let Some(BazelTargetKind::Test) = kind_filter {
            ...
            let bin_or_lib = targets
                .iter()
                .find(|t| matches!(t.kind, BazelTargetKind::Binary | BazelTargetKind::Library));

            if let Some(bin_or_lib_target) = bin_or_lib {
                tracing::debug!(
                    "Found binary/library target {} containing file",
                    bin_or_lib_target.name
                );
                    ...
                    if matches!(target.kind, BazelTargetKind::Test) {
                        ...
                        if let Some(crate_ref) = &target.attributes.crate_ref {
                            ...
                            if crate_name == bin_or_lib_target.name {
                                tracing::debug!(
                                    "Found rust_test target {} that tests {}",
                                    target.label,
                                    bin_or_lib_target.name
                                );
        ...
        let runnable_targets: Vec<_> = targets
            .into_iter()
    ...
    pub fn find_integration_test_target(
        ...
        tracing::debug!(
            "find_integration_test_target: file_path={:?}, workspace_root={:?}",
            file_path,
            workspace_root
        );
    ...
    fn target_includes_file(&self, target: &BazelTarget, file_path: &str) -> bool {
        tracing::debug!(
            "target_includes_file: checking if target {} includes file {}",
            target.name,
            file_path
        );
...
"#;
        ...
        let target = finder
            .find_runnable_target(&bench_file, temp_dir.path(), Some(BazelTargetKind::Test))
            .unwrap();
...
"#;
        ...
        let mut finder = BazelTargetFinder::new().unwrap();
        let targets = finder
            .find_targets_for_file(&test_file, temp_dir.path())
            .unwrap();
...
"#;
        ...
        let target = finder
            .find_integration_test_target(&test_file, temp_dir.path())
            .unwrap();
