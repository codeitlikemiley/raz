        ...
        match CommandTemplate::parse(template) {
            Ok(tmpl) => {
                match tmpl.render(|ph| match ph {
                    ...
                    | "test_name"
                    | "bench_filter" => {
                        if test_filter_owned.is_empty() { None } else { Some(test_filter_owned.clone()) }
                    }
                    ...
                    Err(e) => {
                        tracing::warn!("BazelCommandBuilder: template render failed ({e}), using legacy expand");
                        Self::legacy_expand(
                            template, &target_owned, &target_name_owned, &package_owned,
                            &file_path_str, &file_name_owned, &parent_dir_owned,
                            &test_filter_owned, &module_path_owned, &binary_owned,
                        )
                    }
                }
            }
            Err(e) => {
                tracing::warn!("BazelCommandBuilder: template parse failed ({e}), using legacy expand");
                Self::legacy_expand(
                    template, &target_owned, &target_name_owned, &package_owned,
                    &file_path_str, &file_name_owned, &parent_dir_owned,
                    &test_filter_owned, &module_path_owned, &binary_owned,
