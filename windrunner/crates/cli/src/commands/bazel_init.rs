...
// Root-package file labels use //:file syntax, NOT //file (which is a package name).
...
# The settings below cut download size and speed up cold starts.
...
# ── Restrict Rust toolchain to host platform only ─────────────────────────────
# Without this, rules_rust downloads rust-std for EVERY known target triple.
# Restricting to the host triple saves ~500 MB–1 GB on first sync.
build --@rules_rust//:extra_rustc_flags=
